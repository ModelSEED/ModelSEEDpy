...
``COBRAkbase`` models, either with or without additional constraints, can be simulated through a few FBA processes. A broad example of FBA in ModelSEEDpy is available `here <flux_analysis/FBAExampleNotebook>`_ .
...
The ``MetaboFBA`` Package forces flux through observed metabolites in metabolomics data. An example is available `here <flux_analysis/MetaboFBAExample>`_ .


Flexible Biomass
---------------------

The ``FlexibleBiomass`` Package enables the coefficients on a biomass reaction to vary. An example is available `here <flux_analysis/FlexibleBiomass>`_ .


Media Scan
---------------------

The ``Media Scan`` Package concisely tests numerous media for a single model. An example is available `here <flux_analysis/MediaScan>`_ .
