...
class MSExpression:
    ...
    def build_reaction_expression(self,model,default):
        if self.type == MODEL:
            logger.critical("Cannot build a reaction expression from a model-based expression object!")
        ...
        for gene in model.genes:
            ...
            if feature == None:
                logger.warning("Model gene "+gene.id+" not found in genome of expression")
            elif feature.id not in self.features:
                logger.warning("Model gene "+gene.id+" in genome but not in expression")
            else:
