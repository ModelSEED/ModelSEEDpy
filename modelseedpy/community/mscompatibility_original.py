...
class MSCompatibility():  # TODO convert to a collection of two static methods with helper functions. Docs should be developed as well.
    def __init__(self,
    ...
    def standardize(self, models,                      # the collection of cobrakbase models that will be compared
        ...
        for self.model_index, model in enumerate(self.models):
            with model:
                self.model = model
    ...
    def align_exchanges(self, models,                        # the collection of cobrakbase models that will be compared
        ...
        self.changed_ids_count = self.changed_rxn_count = 0
        if standardize:
            self.standardize(self.models, conflicts_file_name='standardized_exchange_metabolites.json', model_names=model_names)
        ...
        for self.model_index, model in enumerate(self.models):
            ...
            for ex_rxn in self.model.exchanges:
                for met in ex_rxn.metabolites:
                    ...
                    if met.id not in self.unique_mets and met.id not in established_mets: 
                        if met_name not in unique_names:
                            ...
                            former_model_index = remove_prefix(list(self.unique_mets[former_id].keys())[0].split('_')[0], 'model')
                        ...
                        former_model_index = remove_prefix(list(self.unique_mets[met.id].keys())[0].split('_')[0], 'model')
                        if met_name == former_name:
                            ...
                            if met.id not in self.met_conflicts:
                                self.met_conflicts[met.id] = {
                                        ...
                                        f'model{former_model_index}_met': self.unique_mets[met.id][f'model{former_model_index}_met'],
                ...
                if reaction in model_metabolites:
                    ...
                    rxn_met, new_met_id, success = self._fix_met(remove_suffix(reaction, suffix))
    ...
    def __correct_met(self, met, met_name, standardize = False):
        def check_cross_references(met, general_met):
            met_refs = self.compounds_cross_references[self.compound_names[met_name]]
            match = False
        ...
        if remove_suffix(met.id, compartment) != self.compound_names[met_name]:  # If the ID associated with the name deviates from that in the ModelSEED Database
                    ...
                    for rxn_met in rxn.reactants+rxn.products:  # The REACTANTS+PRODUCTS may resolve metabolites that are both, more than the METABOLITES attribute
                        ...
                        if rxn_met.id == met.id:
                            ...
                            if 'cpd' in met.id and self.compounds_cross_references[general_met] != {}:
                                ...
                                if not match:
                                    logger.warning(f'ModelSEEDError: The old metabolite {met.id} cross-references ({self.compounds_cross_references[general_met]}) do not overlap with those ({self.compounds_cross_references[self.compound_names[met_name]]}) of the new metabolite {new_met_id}.')
