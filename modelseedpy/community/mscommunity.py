...
class CommunityModelSpecies:
    def __init__(
        ...
        self.index = int(
            self.biomass_cpd.compartment[1:]
        ...
        atp_rxn = FBAHelper.add_atp_hydrolysis(
            self.community.model, "c" + str(self.index)
        )
        ...
        for rxn in self.community.model.reactions:
            if (
                int(FBAHelper.rxn_compartment(rxn)[1:]) == self.index
                and "bio" not in rxn.name
            ):
    ...
    def compute_max_biomass(self):
        ...
        self.community.model.objective.set_linear_coefficients(
            {self.biomasses[0].forward_variable: 1}
        )
        if self.community.lp_filename != None:
            self.community.print_lp(
                self.community.lp_filename + "_" + self.id + "_Biomass"
            )
...
class MSCommunity:
    def __init__(
        ...
        self.solution = (
            self.biomass_cpd
        ) = self.primary_biomass = self.biomass_drain = None
        self.msgapfill = (
            self.element_uptake_limit
        ...
        self.model = (
            model
            if not models
            else MSCommunity.build_from_species_models(
    ...
    def build_from_species_models(
                        ...
                        if index not in biomass_indices:
                            ...
                            if rxn_id not in model_reaction_ids:
                                ...
                                for i in range(len(models) * 2):
                                    rxn_id = "bio" + str(i)
                                    if (
                                        rxn_id not in model_reaction_ids
                                        and i not in biomass_indices
                                    ):
                                        rxn.id = rxn_id
        ...
        metabolites.update(
            {cpd: -1 / len(biomass_compounds) for cpd in biomass_compounds}
        )
    ...
    def set_objective(
        self, target=None, minimize=False
    ):  #!!! Mustn't a multilevel objective be set for community models?
        if target is None:
        ...
        self.model.objective = self.model.problem.Objective(
            self.model.reactions.get_by_id(target).flux_expression, direction=sense
        )

    def constrain(
        self, element_uptake_limit=None, kinetic_coeff=None, modelseed_db_path=None
    ):
        ...
        if modelseed_db_path:
            self.pkgmgr.getpkg("FullThermoPkg").build_package(
                {"modelseed_db_path": modelseed_db_path}
            )
    ...
    def compute_interactions(
                    ...
                    if metabolite in metabolite_data:
                        if species_list[comp_index] != None:
                            metabolite_data[metabolite][
                                species_list[comp_index].id
                            ] += (solution.fluxes[rxn.id] * rxn.metabolites[metabolite])
        ...
        for met in metabolite_data:
            ...
            total = sum(
                [
                    metabolite_data[met][individual.id]
                    for individual in self.species
                ...
                if metabolite_data[met][individual.id] > Zero:
                    ...
                    if metabolite_data[met]["Environment"] < Zero:
                        normalized_flux = (
                            abs(
                                metabolite_data[met][individual.id]
                                * metabolite_data[met]["Environment"]
                            )
                            / total
                        )
                        species_data[individual.id]["Environment"] += normalized_flux
                        if normalized_flux > threshold:
                            species_collection[individual.id]["Environment"].append(
                                met.name
                            )
            if metabolite_data[met]["Environment"] > Zero:
                for individual in self.species:
                    if metabolite_data[met][individual.id] < Zero:
                        ...
                        if normalized_flux > threshold:
                            species_collection["Environment"][individual.id].append(
                                met.name
                            )
        ...
        data["Environment"].append(0), data["IDs"].append("Environment list"), data[
            "Metabolites/Donor"
        ...
        if visualize:
            self._visualize_cross_feeding(
                export_directory, node_metabolites, x_offset, show_figure
            )

        return self.cross_feeding_df

    def _visualize_cross_feeding(
        self, export_directory, node_metabolites=True, x_offset=0.15, show_figure=True
    ):
        ...
        for species in self.species:
            ...
            for index, entry in net_cross_feeding[
                f"Species{species.index} list"
            ].iteritems():
                if (
                    "Species" in index
                    and re.search("(\d+)", index).group() != species.index
                ):
        ...
        for species_1, species_2 in combinations(list(species_nums.keys()), 2):
            species_2_to_1 = net_cross_feeding.at[
                f"Species{species_2}", f"Species{species_1}"
            ]
            species_1_to_2 = net_cross_feeding.at[
                f"Species{species_1}", f"Species{species_2}"
            ]
            interaction_net_flux = sigfig.round(species_2_to_1 - species_1_to_2, 3)
            self.graph.add_edge(
                species_1, species_2, flux=interaction_net_flux
            )  # The graph plots directionally toward the larger numbered species
        ...
        networkx.draw_networkx_edge_labels(
            self.graph, self.pos, edge_labels=self.labels
        )

        if export_directory:
            pyplot.savefig(os.path.join(export_directory, "cross_feeding_diagram.svg"))
            self.cross_feeding_df.to_csv(
                os.path.join(export_directory, "cross_feeding.csv")
            )
    ...
    def gapfill(
        ...
        gfresults = self.gapfillings[gfname].run_gapfilling(
            media, target, solver=solver
        )
        if not gfresults:
            logger.critical(
    ...
    def test_individual_species(
        self, media=None, allow_cross_feeding=True, run_atp=True, run_biomass=True
    ):
    ...
    def atp_correction(
        ...
        self.atpcorrect = MSATPCorrection(
            self.model,
            core_template,
            atp_medias,
            atp_objective="bio2",
