...
def _name(name, suffix, short_code, timestep, names):
    name = "-".join(
        [x for x in list(map(str, [name + suffix, short_code, timestep])) if x]
    )
    if name not in names:
        ...
        raise ObjectAlreadyDefinedError(
            f"The object {name} is already defined for the problem."
        )
...
def _obj_val(
    primal, name, pheno, short_code, timestep, bounds, data_timestep_hr, names
):
    time_hr = int(timestep) * data_timestep_hr
    return (
        tupVariable(_name(name, pheno, short_code, timestep, names), Bounds=bounds)
        if not primal
        else primal[short_code][name + pheno][time_hr]
    )
    ...
    if kcat_vals:
        ...
        for trial, content in primal_values.items():
            for primal, time_value in content.items():
                ...
                kcat_primal[species][pheno] = clamp(
                    kcat_primal[species][pheno], 1e-4, 10
                )
        return kcat_primal
    select_primals = {}
    for trial, entities in primal_values.items():
        ...
        for entity, times in entities.items():
            ...
            if any([label in entity for label in entity_labels]):
                select_primals[trial][entity] = dict(list(times.items()))
...
def _partition_coefs(initial_val, divisor):
    return (
        initial_val,
        initial_val / divisor,
        initial_val / divisor**2,
        initial_val / divisor**3,
        initial_val / divisor**4,
    )


biomass_partition_coefs = [
    _partition_coefs(10, 10),
    _partition_coefs(2, 2),
    _partition_coefs(1, 3),
]
...
class CommPhitting:
    ...
    def __init__(
        ...
        determine_requisite_biomass: bool = True,
        consumed_mets: iter = None,
    ):
        ...
        self.species_identities_rows = species_identities_rows
        self.significant_deviation = significant_deviation
        self.extract_zip_path = extract_zip_path
        ...
        if community_members is not None or any(
            [x is None for x in [fluxes_df, data_df]]
        ):
            (
                ...
                fluxes_df,
                carbon_conc,
                self.requisite_biomass,
                self.trial_name_conversion,
                self.data_timestep_hr,
                simulation_timestep,
                media_conc,
            ) = GrowthData.process(
                community_members,
        ...
        self.fluxes_df = fluxes_df
        self.data_df = data_df
        self.default_excreta = [
            index for index, row in fluxes_df.iterrows() if any(row > 1)
        ]
    ...
    def fit_kcat(
        ...
        mets_to_track: list = None,
        rel_final_conc: dict = None,
        zero_start: list = None,
        abs_final_conc: dict = None,
        graphs: list = None,
        data_timesteps: dict = None,
        export_zip_name: str = None,
        export_parameters: bool = True,
        requisite_biomass: dict = None,
        export_lp: str = f"solveKcat.lp",
        figures_zip_name: str = None,
        publishing=True,
        ...
        for index, coefs in enumerate(biomass_partition_coefs):
            ...
            newSim = CommPhitting(
                ...
                self.ignore_trials,
                self.ignore_timesteps,
                self.species_identities_rows,
                self.significant_deviation,
                self.extract_zip_path,
                True,
                self.consumed_mets,
            )
            newSim.define_problem(
                parameters,
                mets_to_track,
                rel_final_conc,
                zero_start,
            ...
            kcat_primal = parse_primals(
                newSim.values, coefs=coefs, kcat_vals=newSim.parameters["kcat"]
            )
            pprint(kcat_primal)
    ...
    def define_b_vars(self, pheno, short_code, timestep, variables):
        ...
        variables.extend(
            [
                self.variables["b_" + pheno][short_code][timestep],
        ...
        if short_code not in self.variables[f"bin1_{pheno}"]:
            self.variables[f"bin1_{pheno}"][short_code] = tupVariable(
                _name("bin1_", pheno, short_code, "", self.names),
                Bounds(0, 1),
                "binary",
            )
            self.variables[f"bin2_{pheno}"][short_code] = tupVariable(
                _name("bin2_", pheno, short_code, "", self.names),
                Bounds(0, 1),
                "binary",
            )
            self.variables[f"bin3_{pheno}"][short_code] = tupVariable(
                _name("bin3_", pheno, short_code, "", self.names),
                Bounds(0, 1),
                "binary",
            )
            self.variables[f"bin4_{pheno}"][short_code] = tupVariable(
                _name("bin4_", pheno, short_code, "", self.names),
                Bounds(0, 1),
                "binary",
            )
            self.variables[f"bin5_{pheno}"][short_code] = tupVariable(
                _name("bin5_", pheno, short_code, "", self.names),
                Bounds(0, 1),
                "binary",
            )
            variables.extend(
                [
                    self.variables[f"bin1_{pheno}"][short_code],
                    self.variables[f"bin2_{pheno}"][short_code],
                    self.variables[f"bin3_{pheno}"][short_code],
                    self.variables[f"bin4_{pheno}"][short_code],
    ...
    def define_b_cons(self, pheno, short_code, timestep, biomass_coefs):
        ...
        self.constraints["b1c_" + pheno][short_code][timestep] = tupConstraint(
            _name("b1c_", pheno, short_code, timestep, self.names),
            Bounds(0, None),
            {
                "elements": [
                    {
                    ...
                    },
                    {
                        "elements": [
                            -1,
                            self.variables["b1_" + pheno][short_code][timestep].name,
                        ],
                        "operation": "Mul",
                    },
                ],
        ...
        self.constraints["b2c_" + pheno][short_code][timestep] = tupConstraint(
            _name("b2c_", pheno, short_code, timestep, self.names),
            Bounds(0, None),
            {
                "elements": [
                    {
        ...
        self.constraints["b3c_" + pheno][short_code][timestep] = tupConstraint(
            _name("b3c_", pheno, short_code, timestep, self.names),
            Bounds(0, None),
            {
                "elements": [
                    {
        ...
        self.constraints["b4c_" + pheno][short_code][timestep] = tupConstraint(
            _name("b4c_", pheno, short_code, timestep, self.names),
            Bounds(0, None),
            {
                "elements": [
                    {
        ...
        self.constraints["b5c_" + pheno][short_code][timestep] = tupConstraint(
            _name("b5c_", pheno, short_code, timestep, self.names),
            Bounds(0, None),
            {
                "elements": [
                    {
        ...
        self.constraints["b1c_control_" + pheno][short_code][timestep] = tupConstraint(
            _name("b1c_control_", pheno, short_code, timestep, self.names),
            Bounds(None, 0),
            {
                "elements": [
                    {
                    ...
                    },
                    {
                        "elements": [
                            -1000,
                            self.variables[f"bin1_{pheno}"][short_code].name,
                        ],
                        "operation": "Mul",
                    },
                ],
        ...
        self.constraints["b2c_control_" + pheno][short_code][timestep] = tupConstraint(
            _name("b2c_control_", pheno, short_code, timestep, self.names),
            Bounds(None, 0),
            {
                "elements": [
                    {
        ...
        self.constraints["b3c_control_" + pheno][short_code][timestep] = tupConstraint(
            _name("b3c_control_", pheno, short_code, timestep, self.names),
            Bounds(None, 0),
            {
                "elements": [
                    {
        ...
        self.constraints["b4c_control_" + pheno][short_code][timestep] = tupConstraint(
            _name("b4c_control_", pheno, short_code, timestep, self.names),
            Bounds(None, 0),
            {
                "elements": [
                    {
        ...
        self.constraints["b5c_control_" + pheno][short_code][timestep] = tupConstraint(
            _name("b5c_control_", pheno, short_code, timestep, self.names),
            Bounds(None, 0),
            {
                "elements": [
                    {
        ...
        self.constraints["bin1c_" + pheno][short_code][timestep] = tupConstraint(
            _name("bin1c_", pheno, short_code, timestep, self.names),
            Bounds(0, None),
            {
                "elements": [
                    1000,
        ...
        self.constraints["bin2c_" + pheno][short_code][timestep] = tupConstraint(
            _name("bin2c_", pheno, short_code, timestep, self.names),
            Bounds(0, None),
            {
                "elements": [
                    1000,
        ...
        self.constraints["bin3c_" + pheno][short_code][timestep] = tupConstraint(
            _name("bin3c_", pheno, short_code, timestep, self.names),
            Bounds(0, None),
            {
                "elements": [
                    1000,
        ...
        self.constraints["bin4c_" + pheno][short_code][timestep] = tupConstraint(
            _name("bin4c_", pheno, short_code, timestep, self.names),
            Bounds(0, None),
            {
                "elements": [
                    1000,
        ...
        self.constraints["bin5c_" + pheno][short_code][timestep] = tupConstraint(
            _name("bin5c_", pheno, short_code, timestep, self.names),
            Bounds(0, None),
            {
                "elements": [
                    1000,
        ...
        return [
            self.constraints["b1c_" + pheno][short_code][timestep],
    ...
    def initialize_vars_cons(self, pheno, short_code):
        ...
        self.variables["b1_" + pheno] = {}
        self.variables["b2_" + pheno] = {}
        self.variables["b3_" + pheno] = {}
        self.variables["b4_" + pheno] = {}
        self.variables["b5_" + pheno] = {}
        self.variables["b1_" + pheno][short_code] = {}
        self.variables["b2_" + pheno][short_code] = {}
        self.variables["b3_" + pheno][short_code] = {}
        self.variables["b4_" + pheno][short_code] = {}
        self.variables["b5_" + pheno][short_code] = {}
        ## biomass binary variables
        self.variables[f"bin1_{pheno}"] = {}
        self.variables[f"bin2_{pheno}"] = {}
        self.variables[f"bin3_{pheno}"] = {}
        self.variables[f"bin4_{pheno}"] = {}
        self.variables[f"bin5_{pheno}"] = {}
        self.variables[f"bin1_{pheno}"][short_code] = {}
        self.variables[f"bin2_{pheno}"][short_code] = {}
        self.variables[f"bin3_{pheno}"][short_code] = {}
        self.variables[f"bin4_{pheno}"][short_code] = {}
        ...
        self.constraints["b1c_" + pheno] = {}
        self.constraints["b2c_" + pheno] = {}
        self.constraints["b3c_" + pheno] = {}
        self.constraints["b4c_" + pheno] = {}
        self.constraints["b5c_" + pheno] = {}
        self.constraints["b1c_" + pheno][short_code] = {}
        self.constraints["b2c_" + pheno][short_code] = {}
        self.constraints["b3c_" + pheno][short_code] = {}
        self.constraints["b4c_" + pheno][short_code] = {}
        self.constraints["b5c_" + pheno][short_code] = {}
        self.constraints["b1c_control_" + pheno] = {}
        self.constraints["b2c_control_" + pheno] = {}
        self.constraints["b3c_control_" + pheno] = {}
        self.constraints["b4c_control_" + pheno] = {}
        self.constraints["b5c_control_" + pheno] = {}
        self.constraints["b1c_control_" + pheno][short_code] = {}
        self.constraints["b2c_control_" + pheno][short_code] = {}
        self.constraints["b3c_control_" + pheno][short_code] = {}
        self.constraints["b4c_control_" + pheno][short_code] = {}
        self.constraints["b5c_control_" + pheno][short_code] = {}
        self.constraints[f"binc_{pheno}"] = {}
        self.constraints[f"binc_{pheno}"][short_code] = {}
        self.constraints["bin1c_" + pheno] = {}
        self.constraints["bin2c_" + pheno] = {}
        self.constraints["bin3c_" + pheno] = {}
        self.constraints["bin4c_" + pheno] = {}
        self.constraints["bin5c_" + pheno] = {}
        self.constraints["bin1c_" + pheno][short_code] = {}
        self.constraints["bin2c_" + pheno][short_code] = {}
        self.constraints["bin3c_" + pheno][short_code] = {}
        self.constraints["bin4c_" + pheno][short_code] = {}
        self.constraints["bin5c_" + pheno][short_code] = {}

    def get_timestep_bin(self, timestep):
        if timestep < self.first:
            return 0
        elif timestep < self.second:
            return 1
        elif timestep < self.third:
            return 2
        elif timestep < self.fourth:
            return 3
        return 4
    ...
    def define_problem(
        ...
        self.phenotypes.extend(
            [
                signal_species(signal) + "_stationary"
                for signal in growth_tup.columns
                if (":" in signal and "OD" not in signal)
            ]
        )
        self.species_list = [
            signal_species(signal) for signal in growth_tup.columns if ":" in signal
        ]
        ...
        unique_short_codes = [
            f"{growth_tup.index[0][0]}{num}" for num in map(str, num_sorted)
        ]
        full_times = growth_tup.values[:, growth_tup.columns.index("Time (s)")]
        self.times = {
            short_code: trial_contents(short_code, growth_tup.index, full_times)
            for short_code in unique_short_codes
        }
        average_time_series = np.mean(list(self.times.values()), axis=0)
        points = len(average_time_series)
        ...
        self.time_ranges = {
            0: average_time_series[: self.first],
            1: average_time_series[self.first : self.second],
            2: average_time_series[self.second : self.third],
            3: average_time_series[self.third : self.fourth],
        ...
        self.parameters["data_timestep_hr"] = (
            np.mean(np.diff(np.array(list(self.times.values())).flatten())) / hour
            if not hasattr(self, "data_timestep_hr")
            else self.data_timestep_hr
        )
        self.parameters.update(
            {
                ...
                "kcat": 0.33,
                "diffpos": 1,
                "diffneg": 1,  # coefficients that weight difference between experimental and predicted biomass
        ...
        self.parameters.update(
            self._universalize(self.parameters, "kcat", exclude=["stationary"])
        )
            ...
            for species, content in self.parameters["kcat"].items():
                ...
                for pheno, content2 in content.items():
                    ...
                    for time, val in content2.items():
                        if time not in primal_values[species][pheno]:
                            continue
        ...
        self.zero_start = zero_start or [
            met
            for met in self.consumed_mets
            if (met not in self.carbon_conc or self.carbon_conc[met] == 0)
        ]
        self.rel_final_conc = rel_final_conc or {
            met: 0.1
            for met, concs in self.carbon_conc.items()
            if any(
        ...
        if mets_to_track:
            ...
            self.mets_to_track = list(self.rel_final_conc.keys()) + self.zero_start
        print(self.mets_to_track)

        ts_to_delete = (
            {}
        )  # {short_code: full_times for short_code in unique_short_codes}
        if data_timesteps:  # {short_code:[times]}
            for short_code, times in data_timesteps.items():
                ts_to_delete[short_code] = set(list(range(len(full_times)))) - set(
                    times
                )
                self.times[short_code] = np.delete(
                    self.times[short_code], list(ts_to_delete[short_code])
                )

        # construct the problem
        objective = tupObjective(
            "minimize variance and phenotypic transitions", [], "min"
        )
        ...
        for exID in self.fluxes_tup.index:
            ...
            self.variables[concID] = {}
            self.constraints["dcc_" + met_id] = {}
            ...
            if "Vmax" and "Km" in self.parameters:
                self.parameters["Vmax"].update(
                    self._universalize(self.parameters["Vmax"], met_id)
                )
                self.parameters["Km"].update(
                    self._universalize(self.parameters["Km"], met_id)
                )
            for short_code in unique_short_codes:
                self.variables[concID][short_code] = {}
                self.constraints["dcc_" + met_id][short_code] = {}
                timesteps = list(range(1, len(self.times[short_code]) + 1))
                for timestep in timesteps:
                    ## define the concentration variables
                    conc_var = tupVariable(
                        _name(concID, "", short_code, timestep, self.names)
                    )
                    ...
                    if timestep == timesteps[0]:
                        ...
                        if initial_val is not None:
                            conc_var = conc_var._replace(
                                bounds=Bounds(initial_val, initial_val)
                            )
                            if biolog_simulation:
                                conc_var = conc_var._replace(bounds=Bounds(1, None))
                    ## mandate complete carbon consumption
                    elif timestep == timesteps[-1] and (
                        met_id in self.rel_final_conc or met_id in self.abs_final_conc
                    ):
                        if met_id in self.rel_final_conc:
                            final_bound = (
                                self.variables[concID][short_code][1].bounds.lb
                                * self.rel_final_conc[met_id]
                            )
                        if (
                            met_id in self.abs_final_conc
                        ):  # this intentionally overwrites rel_final_conc
                            final_bound = self.abs_final_conc[met_id]
                        conc_var = conc_var._replace(bounds=Bounds(0, final_bound))
                        if met_id in self.zero_start:
                            conc_var = conc_var._replace(
                                bounds=Bounds(final_bound, final_bound)
                            )
        ...
        for pheno in self.phenotypes:
            for short_code in unique_short_codes:
                ...
                for timestep in map(int, timesteps):
                    ...
                    constraints.extend(
                        self.define_b_cons(pheno, short_code, timestep, biomass_coefs)
                    )
                    ...
                    self.variables["cvt_" + pheno][short_code][timestep] = tupVariable(
                        _name("cvt_", pheno, short_code, timestep, self.names),
                        Bounds(0, 100),
                    )
                    self.variables["cvf_" + pheno][short_code][timestep] = tupVariable(
                        _name("cvf_", pheno, short_code, timestep, self.names),
                        Bounds(0, 100),
                    )
                    variables.extend(
                        [
                            self.variables["cvf_" + pheno][short_code][timestep],
                            self.variables["cvt_" + pheno][short_code][timestep],
                        ]
                    )

                    # cvt <= bcv*b_{pheno} + cvmin
                    self.constraints["cvc_" + pheno][short_code][timestep] = (
                        tupConstraint(
                            _name("cvc_", pheno, short_code, timestep, self.names),
                            (0, None),
                            {
                                "elements": [
                                    {
                    ...
                    self.constraints["cvc_" + pheno][short_code][timestep].expr[
                        "elements"
                    ].extend(biomass_term)
        ...
        time_2 = process_time()
        print(
            f"Done with concentrations and biomass loops: {(time_2 - time_1) / 60} min"
        )
        for r_index, met in enumerate(self.fluxes_tup.index):
            ...
            for short_code in unique_short_codes:
                ...
                for timestep in timesteps[:-1]:
                    ...
                    self.constraints["dcc_" + met_id][short_code][timestep] = (
                        tupConstraint(
                            name=_name(
                                "dcc_", met_id, short_code, timestep, self.names
                            ),
                            expr={
                                "elements": [
                                    ...
                                    *OptlangHelper.dot_product(
                                        growth_phenos,
                                        heuns_coefs=half_dt
                                        * self.fluxes_tup.values[r_index],
                                    ),
                                ],
                                "operation": "Add",
                            },
        ...
        for index, org_signal in enumerate(growth_tup.columns[2:]):
            ...
            species_phenos[species] = {
                None if "OD" in species else f"{species}_stationary"
            }
            ...
            self.variables[signal + "|bio"] = {}
            self.variables[signal + "|diffpos"] = {}
            self.variables[signal + "|diffneg"] = {}
            self.variables["g_" + species] = {}
            self.constraints[signal + "|bioc"] = {}
            self.constraints[signal + "|diffc"] = {}
            self.constraints["gc_" + species] = {}
            self.constraints["totVc_" + species] = {}
            self.constraints["totGc_" + species] = {}
            self.constraints[signal + "|bio_finalc"] = {}
            for short_code in unique_short_codes:
                ...
                values_slice = trial_contents(
                    short_code, growth_tup.index, growth_tup.values
                )
                if ts_to_delete:
                    values_slice = np.delete(
                        values_slice, list(ts_to_delete[short_code]), axis=0
                    )
                ...
                for timestep in timesteps[:-1]:
                    ...
                    if (
                        int(timestep) * self.parameters["timestep_hr"]
                        < data_timestep * self.parameters["data_timestep_hr"]
                    ):
                        print(
                            f"Skipping timestep {timestep} that does not align with the user's timestep"
                        )
                        continue
                    data_timestep += 1
                    if data_timestep > int(
                        self.times[short_code][-1] / self.parameters["data_timestep_hr"]
                    ):
                        print(
                            f"The user-defined time exceeds the simulation time, so the DBC & diff loop is broken."
                        )
                        break
                    ...
                    for pheno in species_phenos[species]:
                        ...
                        if "stationary" in pheno:
                            ...
                            self.constraints["dbc_" + pheno][short_code][timestep] = (
                                tupConstraint(
                                    name=_name(
                                        "dbc_", pheno, short_code, timestep, self.names
                                    ),
                                    expr={
                                        "elements": [*from_sum, *to_sum],
                                        "operation": "Add",
                                    },
                                )
                            )
                        ...
                        self.constraints["dbc_" + pheno][short_code][timestep].expr[
                            "elements"
                        ].extend(biomass_term)
                        constraints.append(
                            self.constraints["dbc_" + pheno][short_code][timestep]
                    ...
                    if not requisite_biomass or any(
                        [
                            timestep != timesteps[-2],
                            signal not in requisite_biomass[short_code],
                        ]
                    ):
                        self.variables[signal + "|bio"][short_code][timestep] = (
                            tupVariable(
                    ...
                    self.variables[signal + "|diffpos"][short_code][timestep] = (
                        tupVariable(
                            _name(signal, "|diffpos", short_code, timestep, self.names),
                            Bounds(0, 100),
                        )
                    )
                    self.variables[signal + "|diffneg"][short_code][timestep] = (
                        tupVariable(
                            _name(signal, "|diffneg", short_code, timestep, self.names),
                            Bounds(0, 100),
                        )
                    )
                    variables.extend(
                    ...
                    self.constraints[signal + "|bioc"][short_code][timestep] = (
                        tupConstraint(
                            name=_name(
                                signal, "|bioc", short_code, timestep, self.names
                            ),
                            expr={
                                "elements": [
                                    {
                                    ...
                                    {
                                        "elements": [
                                            self.variables[signal + "|conversion"].name,
                    ...
                    self.constraints[signal + "|diffc"][short_code][timestep] = (
                        tupConstraint(
                            name=_name(
                                signal, "|diffc", short_code, timestep, self.names
                            ),
                            expr={
                                "elements": [
                                    self.variables[signal + "|bio"][short_code][
                    ...
                    if all([isinstance(val, dict) for val in signal_sum]):
                        self.constraints[signal + "|diffc"][short_code][timestep].expr[
                            "elements"
                        ].extend(signal_sum)
                    else:
                        raise ValueError(
                            f"The {signal_sum} value has unexpected contents."
                        )
                    constraints.append(
                        self.constraints[signal + "|diffc"][short_code][timestep]
                    )
                                ...
                                "elements": [
                                    {
                                        "elements": [
                                            ...
                                            self.variables[f"{signal}|diffpos"][
                                                short_code
                                            ][timestep].name,
                                        ],
                                        "operation": "Mul",
                                    },
                                    {
                                        "elements": [
                                            ...
                                            self.variables[f"{signal}|diffneg"][
                                                short_code
                                            ][timestep].name,
                                        ],
                                        "operation": "Mul",
        ...
        # construct the problem
        self.problem = OptlangHelper.define_model(
            "CommPhitting model", variables, constraints, objective, True
        )
        self.hdf5_name = export_lp.replace(".lp", ".h5")
        self.hdf5_file = File(self.hdf5_name, "w")
        time_5 = process_time()
        print(
            f"Done with constructing the {type(self.problem)} model: {(time_5 - time_4) / 60} min"
        )
        ...
        if export_zip_name:
            ...
            with ZipFile(self.zip_name, "a", compression=ZIP_LZMA) as zp:
                for file in self.zipped_output:
                    zp.write(file)
                    os.remove(file)
                    self.zipped_output.remove(file)
        time_6 = process_time()
    ...
    def compute(
        ...
        figures_zip_name=None,
        publishing=False,
        primals_export_path: str = "primal_values.json",
        remove_empty_plots=False,
    ):
        ...
        for variable, value in self.problem.primal_values.items():
            if "v_" in variable:
                ...
                if value in self.conversion_bounds:
                    warnings.warn(
                        f"The conversion factor {value} optimized to a bound, which may be "
                ...
                self.values[short_code][basename] = self.values[short_code].get(
                    basename, {}
                )
        ...
        if graphs:
            self.graph(
                graphs,
                export_zip_name=figures_zip_name or export_zip_name,
        ...
        for col in values_df.columns:
            ...
            for max_time_series in times:
                if len(max_time_series) == max_time:
                    break
            ...
            for index, ele in enumerate(trial_values):
                ...
                if FBAHelper.isnumber(ele):
                    self.hdf5_file.create_dataset(dataset_name, data=[float(ele)])
                elif isinstance(ele, dict):
                    self.hdf5_file.create_dataset(
                        dataset_name, data=list(map(float, ele.values()))
                    )
                    self.hdf5_file[dataset_name].attrs["full_time"] = (
                        len(ele.values()) == max_time
                    )
    ...
    def load_model(
        self,
        mscomfit_json_path: str = None,
        zip_name: str = None,
        model_to_load: dict = None,
    ):
        if zip_name:
    ...
    def assign_values(param, var, next_dimension, kcat=True):
        ...
        for dim1, dim2_list in next_dimension.items():
            if isinstance(dim2_list, dict):
                dic[var].update(CommPhitting.assign_values(param, dim1, dim2_list))
            else:
                if kcat:
                    dic[var][dim1] = param
                else:
        ...
        if not next_dimension:
            ...
            for organism in self.fluxes_tup.columns:
                ...
                if not tsBin:
                    ...
                    if species in next_dimension:
                        next_dimension[species].update({pheno: self.time_ranges})
                    else:
                        next_dimension[species] = {pheno: self.time_ranges}
        if FBAHelper.isnumber(param):
            return CommPhitting.assign_values(param, var, next_dimension)
        elif FBAHelper.isnumber(param[var]):
            return CommPhitting.assign_values(param[var], var, next_dimension)
        elif isinstance(param[var], dict):
            return {
                var: {
                    dim1: {dim2: param[var][dim1] for dim2 in dim2_list}
                    for dim1, dim2_list in next_dimension.items()
                }
            }
        else:
            logger.critical(
                f"The param (with keys {dic_keys(param)}) and var {var} are not amenable"
    ...
    def _add_plot(
        ...
        xs = (
            xs
            if xs is not None
            else list(map(float, self.values[trial][basename].keys()))
        )
        ys = (
            ys
            if ys is not None
            else list(map(float, self.values[trial][basename].values()))
        )
        if scatter:
    ...
    def graph(
        ...
        timestep_ratio = data_timestep_hr / self.parameters.get(
            "timestep_hr", data_timestep_hr
        )
        ...
        for graph_index, graph in enumerate(graphs):
            ...
            if "painting" not in graph:
                graph["painting"] = {
                    ...
                    "ecoli": {"color": "red", "linestyle": "dashed", "name": "E. coli"},
                    "pf": {
                        "color": "green",
                        "linestyle": "dotted",
                        "name": "P. fluorescens",
            ...
            if "species" in graph and graph["species"] == "*":
                ...
                raise ValueError(
                    f"The specified graph {graph} must define species for which data will be plotted."
                )
            ...
            for trial, basenames in self.values.items():
                ...
                for basename, values in basenames.items():
                        ...
                        if "b_" in basename:
                            ...
                            if (
                                any(
                                    [
                                        x in graph["content"]
                                        for x in ["total", "biomass", "OD"]
                                    ]
                                )
                                or graph["species"] == self.species_list
                            ):  # and not graph["parsed"]:
                                total_biomasses["OD"].append(vals)
                                if "OD" not in graph["content"]:
                                    total_biomasses[species].append(vals)
                        if all(
                            [
                                graph["experimental_data"],
                                "|bio" in basename,
                            ]
                        ):
                            ...
                            if publishing:
                                species_name = (
                                    "total"
                                    if "OD" in signal
                                    else graph["painting"][species]["name"]
                                )
                                label = f"Experimental {species_name} (from {signal})"
                            # print(basename, label, self.values[trial][basename].values())
                            if remove_empty_plots and all(
                                self.values[trial][basename].values() == 0
                            ):
                                print(f"The {basename} is empty and thus is removed.")
                                continue
                            ax, labels = self._add_plot(
                                ax,
                                labels,
                                label,
                                basename,
                                ...
                                x_axis_split,
                                scatter=True,
                                color=self.adjust_color(
                                    graph["painting"][species]["color"], 1.5
                                ),
                            )
                    ...
                    if "phenotype" in graph:
                        ...
                        for specie in graph["species"]:
                            ...
                            ax, labels = self._add_plot(
                                ax, labels, label, basename, trial, x_axis_split, style
                            )
                            if graph["parsed"]:
                    ...
                    # graph media concentration plots
                        ...
                        [
                            any([x in basename for x in graph["mets"]]),
                            "c_cpd" in basename,
                        ]
                    ):
                        if not any(
                            np.array(list(self.values[trial][basename].values()))
                            > mM_threshold
                        ):
                            continue
                        if remove_empty_plots and all(
                            ...
                            continue
                        label = self.msdb.compounds.get_by_id(
                            re.search(r"(cpd\d+)", basename).group()
                        ).name
                        ax, labels = self._add_plot(
                            ax, labels, label, basename, trial, x_axis_split
                        )
                        yscale = "log"
                ...
                if labels:  # assesses whether graph(s) were created
                    ...
                    if any([x in graph["content"] for x in ["OD", "biomass", "total"]]):
                        labeled_species = [
                            label for label in labels if isinstance(label, dict)
                        ]
                        for name, vals in total_biomasses.items():
                            # ic(name)
                            if not vals or (
                                len(total_biomasses) == 2 and "OD" not in name
                            ):
                                continue
                            if len(total_biomasses) == 2:
                                specie_label = [
                                    graph["painting"][name]["name"]
                                    for name in total_biomasses
                                    if "OD" not in name
                                ][0]
                            ...
                            style = (
                                ...
                                if (
                                    "OD" in name
                                    and not graph["experimental_data"]
                                    or "total" in graph["content"]
                                )
                                else style
                            )
                            ...
                            if graph["parsed"]:
                                ...
                                if "phenotype" in graph and not isinstance(
                                    graph["phenotype"], str
                                ):
                                    phenotype_id = (
                                        f"{','.join(graph['phenotype'])} phenotypes"
                                    )
                                fig_name = f'{"_".join([trial, name, phenotype_id, content])}.jpg'
                                fig.savefig(
                                    fig_name, bbox_inches="tight", transparent=True
                                )
                    ...
                    if "mets" not in graph and content != "c_":
                        species_id = (
                            graph["species"]
                            if isinstance(graph["species"], str)
                            else ",".join(graph["species"])
                        )
                    ...
                    if not publishing:
                        if not title:
                            org_content = (
                                content
                                if content not in contents.values()
                                else list(contents.keys())[
                                    list(contents.values()).index(content)
                                ]
                            )
                            ...
                            ax.set_title(title)
                    fig_name = (
                        f'{"_".join([trial, species_id, phenotype_id, content])}.jpg'
                    )
                    if "mets" in graph:
    ...
    def engineering(self):
        ...
        self.problem.objective = Objective(
            sum([x for x in self.problem.variables if "bio" in x.name])
        )
...
class BIOLOGPhitting(CommPhitting):
    def __init__(
        ...
        self.experimental_metadata = experimental_metadata
        self.carbon_conc = carbon_conc
        self.media_conc = media_conc or []
        self.fluxes_df = fluxes_df
        self.phenotypes = list(self.fluxes_df.columns)
        self.phenotypes.extend(
            [
                signal_species(signal) + "_stationary"
                for signal in self.biolog_df
                if ":" in signal
            ]
        )
        self.community_members = community_members
    ...
    def fitAll(
        ...
        model_abbreviations = ",".join(
            [content["name"] for content in self.community_members.values()]
        )
        for exp_index, experiment in self.experimental_metadata.iterrows():
            ...
            for model in self.community_members:
                ...
                if "C" not in cpd.elements or not any(
                    [re.search(pheno, rxn.id) for rxn in model.reactions]
                ):
                    if "valid_condition" not in locals():
                        valid_condition = False
                    continue
                exp_list = [pheno] if isinstance(pheno, str) else pheno
                self.community_members[model].update(
                    {
                        "phenotypes": {
                            re.sub(r"(-|\s)", "", experiment["condition"]): {
                                "consumed": exp_list
                            }
                        }
                    }
            ...
            if not valid_condition:
                print(
                    f"The BIOLOG condition with {experiment['ModelSEED_ID']} is not"
                    f" absorbed by the {model_abbreviations} model(s)."
                )
                continue
            print(
                f"The {experiment['ModelSEED_ID']} ({cpd.formula}) metabolite of the "
                f"{experiment['condition']} condition may feed the {model_abbreviations} model(s)."
            )
            if not any(
                [experiment["ModelSEED_ID"] in pheno for pheno in self.phenotypes]
            ):
                print(e)
                print(
                    f"The {experiment['ModelSEED_ID']} ({cpd.formula}) metabolite of the "
            ...
            export_path = os.path.join(
                os.getcwd(), "BIOLOG_LPs", f"{exp_index}_{','.join(exp_list)}.lp"
            )
            kcat_primal = None
            for coef_index, coefs in enumerate(biomass_partition_coefs):
                    ...
                    set(
                        list(
                            chain.from_iterable(
                                [
                                    content["excretions"]
                                    for content in self.community_members.values()
                                ]
                            )
                        )
                    ),
                ...
                time1 = process_time()
                primals_export_path = (
                    primals_export_path or f"BIOLOG_{experiment['ModelSEED_ID']}.json"
                )
                try:
                    ...
                    print(e)
                kcat_primal = parse_primals(
                    new_simulation.values,
                    coefs=coefs,
                    kcat_vals=new_simulation.parameters["kcat"],
                )
                time2 = process_time()
                print(
                    f"Done simulating with the coefficients for biomass partitions: {coef_index}"
