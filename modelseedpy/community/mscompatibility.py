...
class MSCompatibility():
    ...
    @staticmethod
    def standardize(models:Iterable, unknown_met_ids:Iterable, changed_metabolites:Iterable, changed_reactions:Iterable, 
                    metabolites:bool=True, exchanges:bool=True, conflicts_file_name:str=None, model_names:list=None, export_directory:str=None,
                    view_unknown_mets:bool=True, printing:bool=True):
                    ...
                    if exchanges:
                        ...
                        for ex_rxn in model.exchanges:
                            for met in ex_rxn.metabolites:
                                ...
                                try:  # possibly catching errors of repeated metabolite IDs
        ...
        print(f'\n\n{len(changed_reactions)} reactions were substituted and {(changed_metabolites)} metabolite IDs were redefined.')
    ...
    def align_exchanges(models, standardize:bool=False, conflicts_file_name:str=None, model_names:list=None, export_directory:str=None, printing:bool=True, extras=False): 
        ...
        if standardize:
            models, unknown_met_ids = MSCompatibility.standardize(models, unknown_met_ids, changed_metabolites, changed_reactions, 
                              True, True,'standardized_exchange_metabolites.json', model_names, export_directory, True, printing)
            
        unique_mets, met_conflicts = OrderedDict(), OrderedDict()
        for model_index, model in enumerate(models):
            with model:  # implements the predictability of functional programming
                ...
                for ex_rxn in model.exchanges:
                    ...
                    if reaction in model_metabolites:
                        ...
                        model, met, new_met_id, unknown_met_ids, changed_mets, changed_rxns = MSCompatibility._fix_met(
                            model, model_index, reaction.removesuffix(suffix), unknown_met_ids, 
    ...
    def _fix_met(model, model_index, met, unknown_met_ids, changed_metabolites, changed_reactions, standardize, printing):
        # correct the conflict
        base_name = ''.join(met.name.split('-')[1:]).capitalize()
        met_name = re.sub('_\w\d$', '', met.name)
        new_met_id = met.id
        if met.name in compoundNames:
        ...
        elif met.name.capitalize() in compoundNames:
            model, met, new_met_id, changed_metabolites, changed_reactions = MSCompatibility._correct_met(
                model, model_index, met, met.name.capitalize(), changed_metabolites, changed_reactions, standardize, printing)
        elif met_name in compoundNames:
            model, met, new_met_id, changed_metabolites, changed_reactions = MSCompatibility._correct_met(
                model, model_index, met, met_name, changed_metabolites, changed_reactions, standardize, printing)
        elif met_name.capitalize() in compoundNames:
            model, met, new_met_id, changed_metabolites, changed_reactions = MSCompatibility._correct_met(
                model, model_index, met, met_name.capitalize(), changed_metabolites, changed_reactions, standardize, printing)
        elif base_name in compoundNames and base_name != '':
            model, met, new_met_id, changed_metabolites, changed_reactions = MSCompatibility._correct_met(
                model, model_index, met, base_name, changed_metabolites, changed_reactions, standardize, printing)
        else:
            unknown_met_ids.append(met.id)
    ...
    def _export(models, conflicts,       # the conflicts dictionary that will be exported
    ...
    def _correct_met(model, model_index, old_met, met_name, changed_metabolites, changed_reactions, standardize, printing):
        ...
        with model:
            ...
            if met.id.removesuffix(compartment) != compoundNames[met_name]:  # If the ID associated with the name deviates from that in the ModelSEED Database
                ...
                if new_met_id in met.model.metabolites:
                    ...
                    for rxn in met.reactions:
                            ...
                            if rxn_met.id == met.id:
                                if new_met_id in [old_met.id for old_met in rxn.metabolites]:
                                    ...
                                    logger.warning(f'CodeError: The metabolite {new_met_id} replacement for {met.id} \
                                                       already exists in the reaction {rxn.id}, thus the reaction cannot be updated.')
                                    break
                                ...
                                if 'cpd' in met.id and compounds_cross_references[general_met] != {}:
                                    ...
                                    if not match:
                                        logger.warning(f'ModelSEEDError: The old metabolite {met.id} cross-references \
                                                           ({compounds_cross_references[general_met]}) do not overlap with those \
                                                           ({compounds_cross_references[compoundNames[met_name]]}) of the new metabolite {new_met_id}.')
                                ...
                                if 'EX_' in rxn.id and 'EX_'+new_met_id in [ex_rxn.id for ex_rxn in model.exchanges]:
                                    change = {'original': {'reaction': original_reaction},
                                            'new': {'reaction': None},
                                            'justification': f'A {new_met_id} exchange reaction already exists in model {model_index}, \
                                                thus this duplicative exchange reaction ({rxn.id}) is deleted.'}
                                    if match:
                        ...
                        if num_reactants == new_reactants and num_products == new_products:
                            ...
                            change = {'original': {'reaction': original_reaction},
                                    ...
                                    'justification': f'The {new_met_id} replacement for {met.id} already exists in model {model_index}, \
                                        so each reaction (here {rxn.id}) must be updated.'}
                            if match:
                                change['justification'] += f' The ID match was verified with {db} cross-references.'
                            changed_reactions.append(change)
                            if printing:
                            ...
                            logger.warning(f'CodeError: The reaction {reaction_dict} | {new_reactants} {new_products} possesses \
                                               ...
                                               {original_reaction} | {num_reactants} {num_products}, and is skipped.')
                else:
                    # affirm the match with cross-references, where it is possible for ModelSEED compounds
                    match = False
                    general_met = re.sub("(_\w\d$)", "", met.id)
                    ...
                    change = {'original': {'id': original_id, 'name': met.name},
