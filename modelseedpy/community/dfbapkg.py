...
class dFBAPkg(BaseFBAPkg):
    def __init__(self, 
        ...
        BaseFBAPkg.__init__(
            self, model, "BasedFBA", {"met": "metabolite"}, {"conc": "metabolite"}
        )
    ...
    def simulate(
        ...
        self.cellular_dry_mass_fg: float = cellular_dry_mass_fg * femto
        self.cellular_fL: float = cellular_fL * femto
        self.parameters["timesteps"] = int(total_time / timestep)
        self.timestep_value: float = timestep
        ...
        chemostat_requirements = [
            isnumber(chemostat_L),
            feed_profile != {},
            isnumber(exchange_rate),
        ]
        if any(chemostat_requirements) and not all(chemostat_requirements):
            warn(
                f"The chemostat_L ({chemostat_L}), feed_profile ({feed_profile}), and exchange_rate ({exchange_rate}) parameters must all be defined to simulate a chemostat."
            )
        ...
        for self.timestep in range(1, self.parameters["timesteps"] + 1):
            ...
            if all(chemostat_requirements):
                self.chemical_moles[self.col] = (
                    self.concentrations[self.col] * milli * chemostat_L
                )
            ...
            if self.printing:
                print(
                    f"\nobjective value for timestep {self.timestep}: ",
                    self.solutions[-1].objective_value,
                )
    ...
    def _initial_concentrations(
        ...
        self.col = "0 min"
        self.concentrations = pandas.DataFrame(
            index=set(self.met_names), columns=[self.col]
        )
        self.chemical_moles = pandas.DataFrame(
            index=set(self.met_names), columns=[self.col]
        )
        self.concentrations.index.name = (
            self.chemical_moles.index.name
        ) = "metabolite (\u0394mM)"

        self.fluxes = pandas.DataFrame(
            index=set(rxn.name for rxn in self.model.reactions), columns=[self.col]
        )
                ...
                for var in datum["initial_concentrations_M"]:
                    ...
                    if met_id in self.met_ids:
                        ...
                        if name in self.met_names:
                            self.concentrations.at[name, self.col] += (
                                datum["initial_concentrations_M"][var] / milli
                            )
                            initial_concentrations[met_id] = self.concentrations.at[
                                name, self.col
                            ]
                        else:
                            if self.warnings:
                                warn(
                                    f"KineticsError: The {name} reagent ({var}) in the {datum['substituted_rate_law']} rate law is not defined by the model."
                                )
                    else:
                        if self.warnings:
                            warn(
                                f"KineticsError: The {name} reagent ({var}) in the {datum['substituted_rate_law']} rate law is not recognized by the ModelSEED Database."
                            )

        # incorporate custom initial concentrations
        if (
            isinstance(self.initial_concentrations_M, dict)
            and self.initial_concentrations_M != {}
        ):
            for met_id in self.initial_concentrations_M:
                ...
                if met_name not in self.concentrations.index:
                    if self.warnings:
                        warn(
                            f"InitialConcError: The {met_id} ({met_name}) metabolite is not defined by the model."
                        )
                else:
                    self.concentrations.at[met_name, self.col] = (
                        self.initial_concentrations_M[met_id] * milli
                    )
                    initial_concentrations[met_id] = self.concentrations.at[
                        name, self.col
                    ]
    ...
    def _calculate_kinetics(self):
        for reaction_name in self.kinetics_data:
            ...
            for source in self.kinetics_data[reaction_name]:
                ...
                if (
                    "substituted_rate_law" in datum
                ):  #!!! Statistics of aggregating each condition should be provided for provenance.
                    remainder = re.sub(
                        "([0-9A-Za-z/()e\-\+\.\*])", "", datum["substituted_rate_law"]
                    )
                    if remainder == "":
                            ...
                            if len(var) == 1:
                                conc_dict[var] = (
                                    self.concentrations.at[
                                        self.met_ids[met_id], self.previous_col
                                    ]
                                    * milli
                        ...
                        if conc_dict != {}:
                            ...
                            if "metadata" in self.kinetics_data[reaction_name][source]:
                                add_or_write = self.__find_data_match(
                            ...
                            if self.warnings:
                                warn(
                                    f"MetaboliteError: The {reaction_name} reaction possesses unpredictable chemicals."
                                )
                    else:
                        if self.warnings:
                            warn(
                                'RateLawError: The {datum["substituted_rate_law"]} rate law contains unknown characters: {remainder}'
                            )
                else:
                    if self.warnings:
            ...
            if isnumber(flux):
                if reaction_name in self.defined_reactions:
                    ...
                    if self.warnings:
                        warn(
                            f"ReactionError: The {reaction_name} reaction, with a flux of {flux}, is not described by the model."
                        )
            else:
                if self.warnings:
                    warn(
                        f'FluxError: The {reaction_name} reaction flux {datum["substituted_rate_law"]} value {flux} is not numberic.'
                    )
    ...
    def _visualize(
        ...
        legend_list, times = [], [
            t * self.timestep_value * x_axis_scalar
        ...
        if included_metabolites == []:
            ...
            for chem in self.changed:
                if (
                    max(self.concentrations.loc[[chem]].values[0].tolist()) > 1e-2
                ):  # an arbitrary concentration threshold for plotting on the figure
        ...
        for chem in self.changed:
            if chem in included_metabolites:
                concentrations = (
                    self.concentrations.loc[[chem]].values[0].tolist()
                )  # molar
    ...
    def _export(
        ...
        self.concentrations.to_csv(
            os.path.join(self.simulation_path, "concentrations.csv")
        )

        times = self.fluxes.columns
        with open(
            os.path.join(self.simulation_path, "objective_values.csv"), "w"
        ) as obj_val:
        ...
        for met in self.model.metabolites:
            if met.id in self.initial_concentrations_M:
                ...
                for rxn in met.reactions:
                    stoich = (
                        timestep_hr * rxn.metabolites[met]
                    )  # The product of the reaction stoichiometry and the timestep, for the integration of the steady-state
                ...
                self.constraints["conc"][met.id] = self.model.problem.Constraint(
                    Zero,
                    lb=-self.concentrations.at[met.name, self.previous_col],
                    ub=None,
                    name=f"{met.id}_conc",
                )
    ...
    def _chemostat(
        ...
        for chem_id, conc in feed_profile.items():
            ...
            self.concentrations.at[chem_name, self.col] = (
                self.chemical_moles.at[chem_name, self.col] / milli / chemostat_L
    ...
    def __find_data_match(
        ...
        if isnumber(
            self.kinetics_data[reaction_name][source]["metadata"]["Temperature"]
        ):
            temperature_deviation = (
                abs(
                    self.parameters["temperature"]
                    - float(
                        self.kinetics_data[reaction_name][source]["metadata"][
                            "Temperature"
                        ]
                    )
                )
        ...
        if isnumber(self.kinetics_data[reaction_name][source]["metadata"]["pH"]):
            ph_deviation = (
                abs(
                    self.parameters["pH"]
                    - float(self.kinetics_data[reaction_name][source]["metadata"]["pH"])
                )
                / self.parameters["pH"]
            )
    ...
    def __set_constraints(
        self,
        reaction_name: str,
        flux: float,  # specify the name and flux of the given reaction, respectively
    ):
        ...
        self.model.solver.update()
        self.constrained[rxn_name] = self.model.problem.Constraint(
