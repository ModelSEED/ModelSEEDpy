...
def _create_graph_with_internal_reaction(organismsdata):
    ...
    for modelname in organismsdata:
        ...
        G.add_nodes_from(
            organismsdata[modelname]["reversible_back_rxn_no"], bipartite=1
        )
        irrev_lhs_nodes = list(
            set(
                [
                    item
                    for sublist in organismsdata[modelname]["irreversible_lhs_nodes"]
                    for item in sublist
                ]
            )
        )
        irrev_rhs_nodes = list(
            set(
                [
                    item
                    for sublist in organismsdata[modelname]["irreversible_rhs_nodes"]
                    for item in sublist
                ]
            )
        )
        rev_lhs_nodes = list(
            set(
                [
                    item
                    for sublist in organismsdata[modelname]["reversible_lhs_nodes"]
                    for item in sublist
                ]
            )
        )
        rev_rhs_nodes = list(
            set(
                [
                    item
                    for sublist in organismsdata[modelname]["reversible_rhs_nodes"]
                    for item in sublist
                ]
            )
        )
        ...
        for irrevidx in range(len(organismsdata[modelname]["irreversible_rxn_no"])):
            for lhsmetidx in range(
                len(organismsdata[modelname]["irreversible_lhs_nodes"][irrevidx])
            ):
                G.add_edges_from(
                    [
                        (
                            organismsdata[modelname]["irreversible_lhs_nodes"][
                                irrevidx
                            ][lhsmetidx],
            ...
            for rhsmetidx in range(
                len(organismsdata[modelname]["irreversible_rhs_nodes"][irrevidx])
            ):
                G.add_edges_from(
                    [
                        (
                            organismsdata[modelname]["irreversible_rxn_no"][irrevidx],
                            organismsdata[modelname]["irreversible_rhs_nodes"][
                                irrevidx
                            ][rhsmetidx],
                        )
                    ]
        ...
        for revidx in range(len(organismsdata[modelname]["reversible_rxn_no"])):
            for lhsmetidxrev in range(
                len(organismsdata[modelname]["reversible_lhs_nodes"][revidx])
            ):
                G.add_edges_from(
                    [
                        (
                            organismsdata[modelname]["reversible_lhs_nodes"][revidx][
                                lhsmetidxrev
                            ],
            ...
            for rhsmetidxrev in range(
                len(organismsdata[modelname]["reversible_rhs_nodes"][revidx])
            ):
                G.add_edges_from(
                    [
                        (
                            organismsdata[modelname]["reversible_rxn_no"][revidx],
                            organismsdata[modelname]["reversible_rhs_nodes"][revidx][
                                rhsmetidxrev
                            ],
                        )
                    ]
...
def _create_graph_with_exchange_reactions(G, orgs, namemap):
    ...
    for orgnames in orgs:
        renamed_exc_met = [
            f"{orgnames} {comexcmet}" for comexcmet in common_exchange_metabolite
        ]
        number_exc_met = list(range(0, len(common_exchange_metabolite)))
        mod_exc_rxn_number = [
            f"Org_{orgnames} ER{str(num + 1)}" for num in number_exc_met
        ]
        mod_exc_rev_rxn_number = [
            f"Org_{orgnames} ERR{str(num + 1)}" for num in number_exc_met
        ]
    ...
    for orgnames in orgs:
        ...
        mod_non_common_exc_rxn_number = [
            f"Org_{orgnames} NCER{str(num + 1)}" for num in number_non_common_exc_met
        ]
        mod_non_common_exc_rev_rxn_number = [
            f"Org_{orgnames} NCERR{str(num + 1)}" for num in number_non_common_exc_met
        ]
...
def create_graph(file_names, no_of_orgs):
    ...
    H = []
    organisms_reaction_data, partial_name_map = segregate_reactions_from_models(
        file_names
    )
    if organisms_reaction_data:
        organisms_names = list(organisms_reaction_data.keys())
        all_possible_combis = list(
            combinations(list(range(len(organisms_names))), int(no_of_orgs))
        )
        ...
        if all_possible_combis:
            for ncom in range(len(all_possible_combis)):
                ...
                for numincom in range(len(all_possible_combis[ncom])):
                    current_combination[
                        organisms_names[all_possible_combis[ncom][numincom]]
                    ] = organisms_reaction_data[
                        organisms_names[all_possible_combis[ncom][numincom]]
                    ]
                    file_name = (
                        file_name
...
def find_different_reaction_types(stoi_matrix, model, current_model_name):
    ...
    for rxnidx in range(xdim[0]):
        ...
        total_number_of_metabs_in_reaction.append(
            len(total_metabolites_in_reaction[rxnidx][0])
        )
        ...
        if (
            reaction_in_model[rxnidx][-1] == "b"
        ):  # Assuming the bulk metabolites end in 'b'
            if (
                number_of_reactants_in_reaction[rxnidx] == 1
                and number_of_products_in_reaction[rxnidx] == 1
            ):
                exchange_reaction_idx.append(rxnidx)
        # Case 2 - Presence of exchange metabolites
        elif (
            number_of_reactants_in_reaction[rxnidx] == 1
            and total_number_of_metabs_in_reaction[rxnidx] == 1
        ):
            exchange_reaction_idx.append(rxnidx)
        elif (
    ...
    for excentry in exchange_reaction_idx:
        ...
        if reaction_in_model[excentry][-1] == "b":
            exchange_met_ids.append(
                metabolite_identifiers[np.nonzero(stoi_matrix[excentry])[0][0]]
            )
        else:
            exchange_met_index.append(np.nonzero(stoi_matrix[excentry])[0].tolist()[0])
    ...
    for lhsirridx in range(len(irrev_lhs_temporary)):
        ...
        for met_idx_lhs in irrev_lhs_temporary[lhsirridx]:
            met_namech_lhs = (
                f"{current_model_name} {metabolite_identifiers[met_idx_lhs]}"
            )
    ...
    for rhsirridx in range(len(irrev_rhs_temporary)):
        ...
        for met_idx_rhs in irrev_rhs_temporary[rhsirridx]:
            met_namech_rhs = (
                f"{current_model_name} {metabolite_identifiers[met_idx_rhs]}"
            )
...
def segregate_reactions_from_models(models):
    ...
    for model in models:
        ...
        current_organisms_info[model.id]["total_nodes"] = (
            len(exchange_nodes) + len(irrev_lhs_nodes) + len(rev_lhs_nodes)
        )
...
def find_relievedrxns(model, org_info, org_info_pert):
    relieved = {
        i: list(set(org_info_pert[i]) - set(org_info[i])) for i in org_info_pert
    }
...
def find_stuckrxns(model, community, media, no_of_orgs):
    ...
    if not os.path.exists("results"):
        os.makedirs("results")
    all_possible_combis = list(
        combinations(list(range(len(community))), int(no_of_orgs))
    )
...
def perform_task(
    media, model, transport_rxns, pert_community, org_info_wo_trans_rxn, rem_org_list, n
):
    org_info_pert, scope_pert, namemap_pert = find_stuckrxns(
        model, pert_community, media, len(pert_community)
    )
    org_info_pert = decrypt_orginfo(org_info_pert, namemap_pert)
    org_info_pert_wo_trans_rxn = {
        i: list(set(org_info_pert[i]) - set(transport_rxns)) for i in org_info_pert
    }
...
def write_relieved_rxn_metadata(h, org_info_wo_trans_rxn, org_info_pert_wo_trans_rxn):
    nrelieved = {}
    for i in org_info_pert_wo_trans_rxn:
        nrelieved[i] = len(org_info_pert_wo_trans_rxn[i]) - len(
        ...
        if nrelieved[i]:
            h.write(
                i
                + ","
                + str(len(org_info_wo_trans_rxn[i]))
                + ","
                + str(len(org_info_pert_wo_trans_rxn[i]))
                + ","
                + str(nrelieved[i])
                + "\n"
            )
    ...
    for org1 in model:
        for org2 in model:
            if org1.id + "_" + org2.id in org_info_pair.keys():
                ...
                temp = list(
                    set(org_info_single[org1.id + "_" + org1.id])
                    - set(org_info_pair[org1.id + "_" + org2.id])
                )
                for j in temp:
        ...
        for j in model:
            ...
            if org1 + "_" + org2 in relieved.keys():
                ...
                for rel in relieved[org1 + "_" + org2]:
                    ...
                    rel_rxns_name[org1 + "_" + org2].append(
                        i.reactions[rxn_ids.index(rel)].name
                    )
        ...
        for j in range(len(all_possible_combis[i])):
            ...
            for r in G[i].nodes:
                if r.find(model1) >= 0:
                    rxnNode.append(r)
            for rxn in rxnNode:
                if rxn in visited or rxn.find("ERR") >= 0:
                    continue
                elif rxn.find("Org") >= 0:
                    if (rxn[len(model1) + 5] == "I") or (rxn[len(model1) + 5] == "R"):
                        stuck.append(rxn)
...
def pMSI(models, media):
    ...
    org_info_single, scope_sin, namemap_sin, vis = find_stuck_rxns(
        models, community_model, media, 1
    )
    org_info_pair, scope_pair, namemap_pair, vis = find_stuck_rxns(
        models, models, media, 2
    )
    ...
    for i in org_info_single:
        org_info_single_wo_trans_rxn[i] = list(
            set(org_info_single[i]) - set(comm_util.transport_list())
        )
    for i in org_info_pair:
        org_info_pair_wo_trans_rxn[i] = list(
            set(org_info_pair[i]) - set(comm_util.transport_list())
        )
...
def calculate_higherorderMSI(models, media, clusters="individual_clusters"):
    ...
    org_info_wo_trans_rxn = {
        i: list(set(org_info[i]) - set(comm_util.transport_list())) for i in org_info
    }
    ...
    if clusters == "individual_clusters":
        ...
        for i, model in enumerate(models):
            rem_org_list1[i] = model.id
            rem_org_list2[i] = model.id
    else:
        ...
        for n in rem_org_list1:
            rem_org_list1[n] = [
                cobra.io.read_sbml_model(i).id for i in rem_org_list1[n]
            ]
    ...
    with open(f"results/higher_order_msi.csv", "w") as f:
        for n in rem_org_list1:
            ...
            pert_models, pert_community, pert_model_ids = make_perturbed_community(
                rem_org_list1[n], new_models, new_community
            )
            ...
            if msi:
                relieved, detailed_rel_rxns, rel_rxns_name = find_relievedrxns(
                    pert_models, org_info, org_info_pert
                )
                with open(
                    f"results/clusterKO_/data_analysis/relieved_rxns_Comm--clus{n}.tsv",
                    "w",
                ) as g:
                    write_relieved_rxns(g, relieved, detailed_rel_rxns, rel_rxns_name)
                with open(
                    f"results/clusterKO_/data_analysis/Comm--clus{n}.tsv", "w"
                ) as h:
                    h.write("Comm--clus" + str(n) + "\n")
                    for i in rem_org_list2[n]:
                        h.write(i + "\n")
                    h.write(
                        "num of rxns relieved in the below orgs in the presence of clust"
                        + str(n)
                        + "\n"
                    )
                    h.write("org,unpert,clust_" + str(n) + "KO,rxns relieved\n")
                    write_relieved_rxn_metadata(
                        h, org_info_wo_trans_rxn, org_info_pert_wo_trans_rxn
                    )
            ...
            if not new_community:
                new_community = glob.glob("*.sbml")
            new_community.sort()
            ko_models, ko_community, model_ids = make_perturbed_community(
                pert_model_ids, new_models, new_community
            )
            ...
            if len(ko_org_list) < len(model):
                ...
                if msi:
                    relieved, detailed_rel_rxns, rel_rxns_name = find_relievedrxns(
                        ko_models, org_info, org_info_pert
                    )
                    with open(
                        f"results/clusterKO_/data_analysis/relieved_rxns_Comm--clus{n}.tsv",
                        "w",
                        ...
                        write_relieved_rxns(
                            g, relieved, detailed_rel_rxns, rel_rxns_name
                        )
                    with open(
                        f"results/clusterKO_/data_analysis/Comm{n}--clus.tsv", "w"
                    ) as h:
                        h.write("clus" + str(n) + "--Comm\n")
                        for i in ko_org_list:
                            h.write(i + "\n")
                        h.write(
                            "num of rxns relieved in the below orgs in the presence of Comm"
                        )
