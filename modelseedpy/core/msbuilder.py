...
class MSBuilder:
    ...
    def auto_select_template(self):
        ...
        template_core_map = {'A': 'template_core', 'C': 'template_core', 'N': 'template_core', 'P': 'template_core'}
    ...
    def build_non_metabolite_reactions(self, cobra_model, index='0'): #, allow_all_non_grp_reactions=False): #!!! This function has no purpose here when allow_all_non_grp_reactions=False, hence this function is skipped by `build()` when allow_all_non_grp_reactions=False in its arguments
        reactions_sans_gpr = []
    ...
    def build(self, model_id, index='0', allow_all_non_grp_reactions=False, annotate_with_rast=True):
        if annotate_with_rast:
            self.search_name_to_genes, self.search_name_to_original = _aSearch(self.genome, 'RAST')
