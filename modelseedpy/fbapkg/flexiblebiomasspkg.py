...
class FlexibleBiomassPkg(BaseFBAPkg):
    ...
    def build_package(self,parameters):
            ...
            if msid != "cpd11416" and msid != None:
                if msid in refcpd:
                    ...
                    for curr_class in classes:
                        if self.parameters["use_"+curr_class+"_class"] and msid in classes[curr_class]:
        ...
        for metabolite in self.parameters["bio_rxn"].metabolites:
            ...
            elif met_class[metabolite] != "refcpd" and not self.parameters["use_"+met_class[metabolite]+"_class"]:
                flexcpds.append(metabolite)   
        self.modelutl.add_exchanges_for_metabolites(flexcpds,uptake=1000,excretion=1000,prefix='FLEX_', prefix_name='Biomass flex for ')
            ...
            if self.parameters["use_"+met_class+"_class"]:
                ...
                for msid in class_coef[met_class]:
                    if (
                        ...
                        object_stoichiometry[class_coef[met_class][msid]] = self.parameters[
                            "bio_rxn"
                        ].metabolites[class_coef[met_class][msid]]
    ...
    def build_constraint(self,cobra_obj,obj_type):
        ...
        if obj_type == "flxbio":
            ...
            if obj_type == "flxcpd":
                ...
                first_entry = self.parameters["flex_coefficient"][0]*abs(self.parameters["bio_rxn"].metabolites[cobra_obj])
                second_entry = self.parameters["flex_coefficient"][1]*abs(self.parameters["bio_rxn"].metabolites[cobra_obj])
                object = self.model.reactions.get_by_id("FLEX_"+cobra_obj.id)
            elif cobra_obj.id[0:-5] == None or not self.parameters["use_"+cobra_obj.id[0:-5]+"_class"]:
                return None
            else:
