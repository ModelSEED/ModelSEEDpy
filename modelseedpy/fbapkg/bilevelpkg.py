...
class BilevelPkg(BaseFBAPkg):
    def __init__(self,model):
        BaseFBAPkg.__init__(self,model,"reaction use",
    ...
    def _build_variable(self,obj_type,cobra_obj,obj_coef):
        if obj_type == "dualconst":
            ...
            if cobra_obj.ub == None:
                ub = 0
                coef = cobra_obj.lb
            var = BaseFBAPkg.build_variable(self,obj_type,lb,ub,"continuous",cobra_obj.name)
            obj_coef[var] = coef
            return var
        if obj_type == "dualub":#constrain this variable to zero when the binary variable is zero
            var = BaseFBAPkg.build_variable(self,obj_type,0,1000000,"continuous",cobra_obj.name)
            if re.search('(.+)_(fflxcmp\d+)$', cobra_obj.name) is not None:
                ...
                BaseFBAPkg.build_constraint(self,"dualbin",None,0,{var:1,bvar:-1000000},cobra_obj.name)
            obj_coef[var] = cobra_obj.ub
            return var
        if obj_type == "duallb":
            var = BaseFBAPkg.build_variable(self,obj_type,-1000000,0,"continuous",cobra_obj.name)
            #if re.search('(.+)_(fflxcmp\d+)$', cobra_obj.name) is not None:
        ...
        if obj_type == "flxcmp" and self.parameters["binary_variable_count"] > 0:
            ...
            for i in range(self.parameters["binary_variable_count"]):
                ...
                if cobra_obj.lower_bound < 0:
                    ...
                    var = BaseFBAPkg.build_variable(self,"rflxcmp"+str(i),0,-1*value*cobra_obj.lower_bound/denominator,"continuous",cobra_obj)
                    coefs[0][var] = -1
                if cobra_obj.upper_bound > 0:
                    self.add_variable_type("fflxcmp"+str(i),"reaction")
                    var = BaseFBAPkg.build_variable(self,"fflxcmp"+str(i),0,value*cobra_obj.upper_bound/denominator,"continuous",cobra_obj)
                    coefs[1][var] = -1
            if cobra_obj.lower_bound < 0:
        ...
        if obj_type == "bflxcmp" and self.parameters["binary_variable_count"] > 0:
            for i in range(0,self.parameters["binary_variable_count"]):
                if cobra_obj.lower_bound < 0:
                    ...
                    var = BaseFBAPkg.build_variable(self,"brflxcmp"+str(i),0,1,"binary",cobra_obj)
                    othervar = self.variables["rflxcmp"+str(i)][cobra_obj.id]
                    self.add_constraint_type("brflxcmpc"+str(i),"reaction")
                    BaseFBAPkg.build_constraint(self,"brflxcmpc"+str(i),None,0,{othervar:1,var:-1000},cobra_obj)
                if cobra_obj.upper_bound > 0:
                    self.add_variable_type("bfflxcmp"+str(i),"reaction")
                    var = BaseFBAPkg.build_variable(self,"bfflxcmp"+str(i),0,1,"binary",cobra_obj)
    ...
    def _build_constraint(self,obj_type,cobra_obj,objective,objterms,coefficients):
        if obj_type == "dualvar":
            ...
            if cobra_obj.lb == 0:
                ...
                lb = None
            return BaseFBAPkg.build_constraint(self,obj_type,lb,ub,coef,cobra_obj.name)
            ...
            return BaseFBAPkg.build_constraint(self,obj_type,0,0,coef,"dualobjconst")