...
class FluxFittingPkg(BaseFBAPkg):
    ...
    def build_package(self,parameters):
        ...
        if self.parameters["totalflux"] == 0:
            self.pkgmgr.getpkg("RevBinPkg",1).build_package(self.parameters["target_flux"])
        else:
            self.pkgmgr.getpkg("TotalFluxPkg",1).build_package(self.parameters["target_flux"])
        objvars = []
        for rxnid in self.parameters["target_flux"]:
            if rxnid in self.model.reactions:
                rxnobj = self.model.reactions.get_by_id(rxnid)
                var = self.build_variable(self,"vfit",-1000,1000,"continuous",rxnobj)
